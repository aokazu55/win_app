class ServicesController < ApplicationController
  ...
  before_action :set_service, only: [:show, :edit, :update, :destroy]
    ...
    def show
      @comment = Comment.new
      @comments = @service.comments.order(created_at: :asc)
      # @comments = @service.comments
      # # @comments = @service.comments.paginate(page: params[:page], per_page: 5)　#paginateのために記述
      # # @comment = current_user.comments.find_by(service_id: @service.id)
